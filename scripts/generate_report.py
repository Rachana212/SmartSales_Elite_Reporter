...
def generate_report(start_date_str, end_date_str):
    ...
    # Load data from SQLite for date range
    ...
    # Format data as plain text (no markdown)
...
"""
    ...
    report_path = os.path.join("reports", f"report_{start_date_sql}to{end_date_sql}.txt")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(full_report)

    print(f"Report saved: {report_path}")

    # Create bar chart data: sum sales by Product
    # Find sales column (assume similar logic to app.py or just 'Amount')
    possible_sales_cols = ["Sales", "Amount", "Total", "Revenue"]
    sales_col = None
    for col in df.columns:
        if col.strip().lower() in [name.lower() for name in possible_sales_cols]:
            sales_col = col
            break
...
def create_sales_chart(data, start_date_sql, end_date_sql):
    ...
    os.makedirs("reports", exist_ok=True)
    chart_path = os.path.join("reports", f"chart_{start_date_sql}to{end_date_sql}.png")
